...
Plot the fluorescence intensity in each fluorophore/dye channel. It looks pretty messy here, but we expect to see a bimodal distribution of negative and positive wells for each channel. We'll do some filtering and corrections so that we only analyze the wells that filled and imaged without error.
